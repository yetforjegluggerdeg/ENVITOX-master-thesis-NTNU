...
\begin{table}[H]
	\centering
	\caption{Parameter estimates of the fitted mixed logistic regression model and their 95\% confidence intervals. Marginal and conditional R$^{2}$ are presented as parameters of model fit, together with the model's residual deviance.}
...
Given the that the proportion of aggregated haemocytes is 0.50 when the log odds is equal to zero, Fig. \ref{fig:LogOdds} shows that 1/2 of the haemocytes withdrawn into MPSS had aggregated within 23 minutes post-withdrawal. This interpretation is valid for mussels where the random effects of time; $\gamma_{0i}$ and $\gamma_{1i}$ = 0. For samples withdrawn into ACB or MAS on the other hand, the cells did not achieve this degree of aggregation until 45 and 46 minutes post-withdrawal, respectively. The three log odds curves depicted in Fig. \ref{fig:LogOdds} shows that the inhibitory effects of ACB and MAS were modelled through their significantly lower y-intercepts compared to the MPSS sub-model (Table \ref{tb:regression_table}). Since these estimates were significant after accounting for within-mussel correlations (<0.0001), we can conclude that EDTA is an effective haemocyte anticoagulant on the short term.
...
\subsection{Haemocyte medium: effect on viability}
 The mean percentages of necrotic hemocytes after 15 min, 2 hours and 20 hours incubation in MPSS, ACB and MAS are presented in Figure \ref{fig:BufferViability}, with error bars representing 95\% confidence intervals around group means.

\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/Method development/buffer viability bargraph scaled.pdf}
    \caption{The mean percentages of TO-PRO$^{TM}$-3 Iodide positive hemocytes after 15 minute, 2 hours and 20 hours incubation in Marine Physiological Saline Solution (\, \protect\lysegraabox, \ n=8), Anticoagulant Buffer (\, \protect\customgraybox, \ n=8) and Modified Alsever's Solution (\, \protect\darkgraybox, \ n=8). Error bars represent 95\% confidence intervals around group means, asterisks *, **, *** above error bars denotes confidence level of Welch Two Sample t-test comparisons with the MPSS control group, while asterisks above horizontal lines represent ACB vs. MAS comparisons.}
    \label{fig:BufferViability}
\end{figure}

The differences between the negative control group (MPSS) and the two EDTA-containing buffers were not significantly different from zero after 15 minute incubation. There was however a marginal difference between MAS and ACB, where samples incubated in MAS had 0.17\% less necrotic cells on average compared to those kept in ACB for 15 minutes. This relationship was consistent across all three incubation periods, allthough the differences after 2 and 20 hours were not significantly different from zero (Fig. \ref{fig:BufferViability}).

Moreover, as the incubation period was increased to 2 and 20 hours, the percentages of necrotic haemocytes in ACB and MAS increased relative to the negative control group. The differences were significantly / or report the actual differences with confidence intervals.
