...
\begin{table}[H]
	\centering
	\caption{The proportion of aggregated haemocytes were modeled as a function of log (time) from haemolymph withdrawal, with buffer (MPSS, ACB, MAS) as categorical explanatory variable. The model (\ref{eq:logit}) was fitted as a glm() function with "quasibinomial" error distribution and "logit" link function, as shown in code listing 3.1. Parameter estimates with 95\% CI, standard errors, t-value and the belonging p-value are presented.}
	\label{tb:loglogistic_aggregation}
	\resizebox{\linewidth}{!}{
	\begin{tabular}{cccccc}
        \toprule
	\textbf{Parameter} & \textbf{Estimate} & \textbf{95\% CI} & \textbf{Std. Error} & \textbf{t-value} & \textbf{Pr(T > $\mid$ t $\mid)$} \\
		\midrule
  \emph{$\alpha_{1}$} & -1.3874 &  [-3.7768, -0.3240] & 1.0211 & -1.359 & 0.18 \\
  \emph{$\beta_{1}$}  &  0.4573 &  [0.1777, 1.1333]   & 0.2880 &  1.588 & 0.12\\
  \emph{$\alpha_{2}$} & -4.3905 & [-6.2053, -1.7100]  & 1.2051 & -3.643 & 0.000445\\
  \emph{$\beta_{2}$}  &  1.2175 & [0.4550, 1.7330]    & 0.3427 &  3.553 & 0.000604 \\
  \emph{$\alpha_{3}$} & -5.0427 & [ -6.8589, -2.3669] & 1.2033 & -4.191 & < 0.0000638 \\
  \emph{$\beta_{3}$}  & 1.3146 & [0.5638, 1.8086]     & 0.3356 & 3.918  & < 0.000172 \\
